...
export function Footer() {
  ...
  return (
    <footer className="bg-black border-t border-amber-500/20 overflow-x-hidden">
      {/* Newsletter Section */}
      <div className="container max-w-6xl mx-auto px-4 sm:px-6 md:px-8 lg:px-12 py-10 sm:py-12 md:py-14">
        <div className="max-w-2xl mx-auto text-center">
          <GoldHeart className="mx-auto mb-4" size={50} />
          <h3 className="text-xl sm:text-2xl font-bold text-amber-400 mb-2">{t('newsletter')}</h3>
          <p className="text-base sm:text-lg text-gray-400 mb-4">{t('newsletterDescription')}</p>
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col sm:flex-row gap-3 max-w-md mx-auto">
            ...
            <Button
              type="submit"
              disabled={isSubmitting}
              className="min-h-[44px] min-w-[44px] bg-gradient-to-r from-amber-500 to-yellow-600 hover:from-amber-600 hover:to-yellow-700 text-black font-bold shadow-[0_0_20px_rgba(251,191,36,0.3)] disabled:opacity-50 disabled:cursor-not-allowed"
            >
      ...
      {/* Sitemap: Quick Links + Legal + Sitemap XML – stack on mobile, 2–3 cols on larger */}
      <div className="border-t border-amber-500/20 py-8 sm:py-10">
        <div className="container max-w-6xl mx-auto px-4 sm:px-6 md:px-8 lg:px-12">
              ...
              <ul className="space-y-1">
                {quickLinks.map(({ key, path }) => (
                  <li key={key}>
                    <Link
                      href={path}
                      className="inline-block py-3 min-h-[44px] min-w-[44px] flex items-center text-gray-400 hover:text-amber-400 transition-colors text-sm sm:text-base -my-1"
                    >
                      {tNav(key)}
                    </Link>
                  </li>
      ...
      {/* Social badges – 44px+ touch targets */}
      <div className="border-t border-amber-500/20 py-6 sm:py-8">
        <div className="container max-w-6xl mx-auto px-4 sm:px-6 md:px-8 lg:px-12">
          <div className="flex flex-wrap justify-center gap-4 sm:gap-5 mb-6 sm:mb-8">
            {socialLinks.map(({ name, url, icon: Icon }) => (
              <a
                ...
                target="_blank"
                rel="noopener noreferrer"
                className="min-w-[48px] min-h-[48px] w-12 h-12 rounded-full border-2 border-gray-600 flex items-center justify-center text-gray-400 hover:border-amber-400 hover:text-amber-400 hover:shadow-[0_0_15px_rgba(251,191,36,0.4)] hover:scale-110 transition-all duration-300"
                aria-label={name}
              >
          ...
          {/* Copyright + Legal – stack on mobile, tap-friendly */}
          <div className="flex flex-col sm:flex-row justify-center items-center gap-4 sm:gap-6 text-sm text-gray-500">
            <span className="text-center sm:text-left">© 2026 Goldie XO. {t('rights')}.</span>
            <div className="flex flex-wrap justify-center gap-4 sm:gap-6">
              <Link href="/privacy" className="min-h-[44px] min-w-[44px] inline-flex items-center justify-center py-2 hover:text-amber-400 transition-colors">
                {t('privacy')}
              </Link>
              <Link href="/terms" className="min-h-[44px] min-w-[44px] inline-flex items-center justify-center py-2 hover:text-amber-400 transition-colors">
                {t('terms')}
              </Link>
            </div>
          </div>
